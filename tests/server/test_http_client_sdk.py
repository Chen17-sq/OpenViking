...
async def test_sdk_find_accepts_tags(http_client):
    ...
    result = await client.find(query="sample document", limit=5, tags=["project-a"])
    assert hasattr(result, "resources")
...
async def test_sdk_search_accepts_tags(http_client):
    ...
    result = await client.search(query="sample document", limit=5, tags=["project-a"])
    assert hasattr(result, "resources")
...
async def test_sdk_set_tags_accepts_tags(http_client):
    ...
    result = await client.set_tags(file_uri, ["project-a"], wait=False)
    assert isinstance(result, dict)
